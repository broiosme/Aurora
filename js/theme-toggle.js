...
function updateToggleIcon(btn, theme) {
    ...
    const labelText = theme === "light" ? "Mode Malam" : "Mode Pantai";
    ...
    if (isDockItem) {
        ...
        btn.innerHTML = `<span class="dock-icon">${iconSvg}</span><span class="${tooltipClass}">${labelText}</span>`;
    } else {
        const emoji = theme === "light" ? "🌙" : "☀️";
        btn.innerHTML = `<span>${emoji}</span><span class="${tooltipClass}">${labelText}</span>`;
    }
}
